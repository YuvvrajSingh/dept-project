...
for (const r of renames) {
  ...
  if (!subject) {
    console.log(`  – [${r.oldCode}] not found in DB, skipping rename`);
    continue;
  }
  ...
  if (patch.ok)
    console.log(`  ✓ Renamed [${r.oldCode}] → [${r.newCode}] ${r.name}`);
...
for (const code of toDelete) {
  ...
  if (!subject) {
    console.log(`  – [${code}] not in DB, nothing to delete`);
    continue;
  }
  const del = await fetch(`${BASE}/api/subjects/${subject.id}`, {
    method: "DELETE",
  });
  if (del.ok || del.status === 204)
    console.log(`  ✓ Deleted [${code}] ${subject.name}`);
